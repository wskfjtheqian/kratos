...
func main() {
	conf := Configuration{
		...
		OmitemptyPrefix: flags.String("omitempty_prefix", "", "omit if google.api is empty"),
	}
	...
	opts.Run(func(plugin *protogen.Plugin) error {
		...
		if *conf.OutputMode == "source_relative" {
			for _, file := range plugin.Files {
				...
				gen := NewOpenAPIv3Generator(plugin, conf, []*protogen.File{file})
			...
			return NewOpenAPIv3Generator(plugin, conf, plugin.Files).Run(outputFile)
		}
		return nil
	})
}
